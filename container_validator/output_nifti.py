...
def check_output_is_nifti(output_path: Path, subject_id: str) -> tuple[TestResult, Any]:
    ...
    try:
        ...
        return TestResult("output_is_nifti", False, f"No output file at {output_path}", subject_id), None
    except Exception as exc:
        return TestResult("output_is_nifti", False, f"nibabel.load failed: {exc}", subject_id), None
    ...
    return TestResult("output_is_nifti", True, f"Valid NIfTI, shape {img.shape}", subject_id), img
...
def check_output_shape_matches_any_input(
    ...
    if not loaded_shapes:
        return TestResult(
            "output_shape_matches_any_input", False,
    ...
    return TestResult(
        ...
        ok,
        f"Output shape {out_shape} matches an input shape" if ok
...
def check_output_dtype_is_integer(img: Any, subject_id: str) -> TestResult:
    ...
    if np.issubdtype(data.dtype, np.integer):
        return TestResult("output_dtype_is_integer", True, f"Integer dtype: {data.dtype}", subject_id)
    if np.issubdtype(data.dtype, np.floating):
        if np.array_equal(data, np.round(data)):
            return TestResult(
                "output_dtype_is_integer", True,
                f"Float dtype {data.dtype} but integer-valued",
                subject_id,
            )
        return TestResult(
            "output_dtype_is_integer", False,
...
def check_output_labels_in_range(img: Any, subject_id: str, max_label: int) -> TestResult:
    import numpy as np
    data = np.asanyarray(img.dataobj)
    if data.size == 0:
        return TestResult("output_labels_in_range", False, "Output volume is empty", subject_id)
    obs_min, obs_max = int(np.min(data)), int(np.max(data))
    if obs_min < 0:
        return TestResult(
            "output_labels_in_range", False,
