...
def check_output_is_numpy_file(output_path: Path, subject_id: str) -> tuple[TestResult, Any]:
    try:
        import numpy as np
    except ImportError as exc:
        r = TestResult(
            "output_is_numpy_file", False,
    ...
    try:
        ...
        return TestResult("output_is_numpy_file", False, f"np.load failed: {exc}", subject_id), None
...
def check_output_is_float_array(arr: Any, subject_id: str) -> TestResult:
    ...
    return TestResult(
        ...
        ok,
        f"dtype={arr.dtype}" if ok else f"Expected floating-point dtype, got {arr.dtype}",
        subject_id,
    )
...
def check_output_is_1d_vector(arr: Any, subject_id: str) -> TestResult:
    ...
    return TestResult(
        "output_is_1d_vector",
        ok,
        f"1-D vector with {squeezed.shape[0]} elements" if ok
...
def check_embedding_dim_consistency(outputs: dict[str, Path]) -> TestResult:
    ...
    for sid, path in outputs.items():
        try:
            ...
            return TestResult(
                "embedding_dim_consistency", False,
